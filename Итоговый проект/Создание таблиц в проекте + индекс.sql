...
CREATE DATABASE Educational_base_homework
...
--Это проверка, что находится в таблицах--
...
--Удаляю данные, для проверки работы интеграции-- 

DELETE from dbo.Directory_of_cities
DELETE from dbo.Directory_of_types_of_insurance
DELETE from dbo.header
DELETE from dbo.body
...
--Создание внешнего ключа - есть создание кластеризованного индекса. Поэтому отдельно индекс создаваться не будет по этому полю. (header.DWH_ID - body.primery_id)

--Все данные в этом проекте загружаются через SSIS пакет (своеобразный ETL процессинг) 


--Далее, идут скрипты, которые учавствуют в загрузке данных носителей--
